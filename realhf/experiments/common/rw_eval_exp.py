...
class RWEvalConfig(CommonExperimentConfig):
    ...
    def _heuristic_rpc_allocation(self):
        rwTrain = RPCAllocation(
                rpc=self.rpcs["rwTrain"],
                device_mesh=DeviceMesh(
                    n_nodes=1,
                    n_gpus_per_node=8,
                    mapping=np.array([[1, 1, 1, 1, 1, 1, 1, 1]], dtype=np.int32),
...
register_quickstart_exp("rw-eval", RWEvalConfig)
