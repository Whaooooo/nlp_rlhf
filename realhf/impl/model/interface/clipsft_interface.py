...
from realhf.impl.model.utils.functional import (
    ...
    gather_packed_shifted_log_probs_with_clip,
)


def compute_packed_clip_sft_loss(
    logits: torch.Tensor,
    ...
    logprobs = gather_packed_shifted_log_probs_with_clip(
        logits, cu_seqlens, packed_input_ids, clip_ratio
    ).float()
...
class ClipSFTInterface(model_api.ModelInterface):
    ...
    def train_step(self, model: model_api.Model, x: SequenceSample, n_mbs=None) -> Dict:
        ...
        stat = module.train_batch(
            input_=x,
            loss_fn=lambda input_logits, input_input: compute_packed_clip_sft_loss(input_logits, input_input, clip_ratio=self.clip_ratio),
    ...
    def evaluate(
        ...
        for step, x in enumerate(tqdm.tqdm(eval_dataloader)):
            ...
            res = module.eval_batch(
                input_=x.cuda(),
                loss_fn=compute_packed_clip_sft_loss,
...
model_api.register_interface("clip-sft", ClipSFTInterface)
