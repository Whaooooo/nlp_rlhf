...
logger = logging.getLogger("PackedMPPOInterface")
...
def _mppo_actor_loss_from_model_outputs(
    ...
    """Loss function for mppo actor step, all inputs should be splitted into
    ...
    mppo_loss_mask = input_.data["mppo_loss_mask"]
    ...
    n_tokens = mppo_loss_mask.count_nonzero()
    ...
    loss, mppo_stat = cppo_functional.actor_loss_fn(
        logprobs=logprobs[:middle_len],
        old_logprobs=old_logp,
        advantages=advantages,
        eps_clip=eps_clip,
        loss_mask=mppo_loss_mask,
    )
    ...
    importance_weight = mppo_stat["importance_weight"].float() * n_tokens
    clip_ratio = mppo_stat["clip_ratio"].float() * n_tokens
    approx_kl = mppo_stat["approx_kl"].float() * n_tokens

    # Logging and early stopping according to KL (logp vs ref) or importance ratio (new logp vs old logp).
    mean_ref_kl = (kl_rewards.detach().float() * mppo_loss_mask).sum()
    logging_loss = torch.where(mppo_loss_mask, loss.detach().float(), 0.0).sum()
...
class MPPOActorInterface(model_api.ModelInterface):
    n_minibatches: int = 4
    ...
    def __post_init__(self):
        ...
        self.cost1_avg = ExponentialRunningMeanStd(beta=self.dual1_beta, epsilon=self.dual1_epsilon)
        self.cost2_avg = ExponentialRunningMeanStd(beta=self.dual2_beta, epsilon=self.dual2_epsilon)
    ...
    def train_step(
        ...
        cost1_score = input_.data["costs1"].float()
        cost1_values = input_.data["cost1_values"].float()

        cost2_score = input_.data["costs2"].float()
        cost2_values = input_.data["cost2_values"].float()
        ...
        if self.value_norm:
            ...
            denormalized_cost1_values = self.rms.denormalize(cost1_values)
            denormalized_cost2_values = self.rms.denormalize(cost2_values)
        else:
            denormalized_rew_values = rew_values
            denormalized_cost1_values = cost1_values
            denormalized_cost2_values = cost2_values
        ...
        for i in range(seq_no_eos_mask.shape[0]):
            if not seq_no_eos_mask[i]:
                ...
                denormalized_cost1_values[cu_seqlens[i + 1] - 1] = 0.0
                denormalized_cost2_values[cu_seqlens[i + 1] - 1] = 0.0
                rew_values[cu_seqlens[i + 1] - 1] = 0.0
                cost1_values[cu_seqlens[i + 1] - 1] = 0.0
                cost2_values[cu_seqlens[i + 1] - 1] = 0.0
        ...
        cost1_kl_rewards, costs1 = cppo_functional.get_packed_rewards(
            kl_ctl=self.kl_adapter.value,
            clip_reward_value=self.max_cost_clip,
            log_probs=old_logp,
            ref_log_probs=ref_logp,
            reward_score=cost1_score,
            short1cu_seqlens=short1cu_seqlens,
            seq_no_eos_mask=all_false_seq_no_eos_mask,
        )
        cost2_kl_rewards, costs2 = cppo_functional.get_packed_rewards(
            kl_ctl=self.kl_adapter.value,
            clip_reward_value=self.max_cost_clip,
            log_probs=old_logp,
            ref_log_probs=ref_logp,
            reward_score=cost2_score,
            short1cu_seqlens=short1cu_seqlens,
            seq_no_eos_mask=all_false_seq_no_eos_mask,
        )
        ...
        cost1_advantages, cost1_returns = cppo_functional.get_packed_advantages_and_returns(
            gamma=self.discount,
            lam=self.gae_lambda,
            values=denormalized_cost1_values,
            rewards=costs1,
            short1cu_seqlens=short1cu_seqlens,
            seq_no_eos_mask=all_false_seq_no_eos_mask,
        )
        cost2_advantages, cost2_returns = cppo_functional.get_packed_advantages_and_returns(
            gamma=self.discount,
            lam=self.gae_lambda,
            values=denormalized_cost2_values,
            rewards=costs2,
            short1cu_seqlens=short1cu_seqlens,
            seq_no_eos_mask=all_false_seq_no_eos_mask,
        )
        kl_rewards = ((rew_kl_rewards + cost1_kl_rewards * self.dual1_ratio + cost2_kl_rewards * self.dual2_ratio) / (1 + self.dual1_ratio + self.dual2_ratio)).float()
        returns = ((rew_returns + cost1_returns * self.dual_ratio + cost2_returns * self.dual2_ratio) / (1 + self.dual1_ratio + self.dual2_ratio)).float()
        advantages = ((rew_advantages + cost1_advantages * self.dual1_ratio + cost2_advantages * self.dual2_ratio) / (1 + self.dual1_ratio + self.dual2_ratio)).float()

        if self.dual1_target is None:
            old_mean = self.cost1_avg.mean_std()[0]
            self.cost1_avg.update(cost1_score, ~ all_false_seq_no_eos_mask)
            self.dual1_ratio = (self.dual1_ratio * torch.exp( - self.dual1_lr * (self.cost1_avg.mean_std()[0] - old_mean))).clamp(1e-4, 1e4)
        else:
            self.cost1_avg.update(cost1_score, ~ all_false_seq_no_eos_mask)
            self.dual1_ratio = (self.dual1_ratio * torch.exp( - self.dual1_lr * (self.cost1_avg.mean_std()[0] - self.dual1_target))).clamp(1e-4, 1e4)
        if self.dual2_target is None:
            old_mean = self.cost2_avg.mean_std()[0]
            self.cost2_avg.update(cost2_score, ~ all_false_seq_no_eos_mask)
            self.dual2_ratio = (self.dual2_ratio * torch.exp( - self.dual2_lr * (self.cost2_avg.mean_std()[0] - old_mean))).clamp(1e-4, 1e4)
        else:
            self.cost2_avg.update(cost2_score, ~ all_false_seq_no_eos_mask)
            self.dual2_ratio = (self.dual2_ratio * torch.exp( - self.dual2_lr * (self.cost2_avg.mean_std()[0] - self.dual2_target))).clamp(1e-4, 1e4)
        ...
        task_cost1 = cost1_score.sum()
        task_cost2 = cost2_score.sum()
        ...
        dist.all_reduce(task_cost1, group=constants.data_parallel_group())
        dist.all_reduce(task_cost2, group=constants.data_parallel_group())
        ...
        global_stats = dict(
            ...
            task_cost1=float(task_cost1 / _n_seqs),
            task_cost2=float(task_cost2 / _n_seqs),
            task_dual1_ratio=float(self.dual1_ratio),
            task_dual2_ratio=float(self.dual2_ratio),
        ...
        # Run mini-batched MPPO training!
        ...
        for data in datas:
            stats = module.train_batch(
                ...
                loss_fn=functools.partial(
                    _mppo_actor_loss_from_model_outputs,
        ...
        # FIXME: It only logs the MoE aux loss of the final MPPO mini-batch.
        global_stats.update(
        ...
        if train_stats:
            train_stats = dict(
                mppo_approx_kl=float(train_stats["mppo_approx_kl"] / _n_tokens),
...
@dataclasses.dataclass
class MPPOGTRewardInterface(model_api.ModelInterface):
    enable_save: bool = True
...
@dataclasses.dataclass
class MPPORewardInterface(model_api.ModelInterface):
...
model_api.register_interface("mppo_actor", MPPOActorInterface)
model_api.register_interface("mppo_rw", MPPORewardInterface)
model_api.register_interface("mppo_gt_rw", MPPOGTRewardInterface)