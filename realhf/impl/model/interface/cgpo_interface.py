...
logger = logging.getLogger("PackedCGPOInterface")


def _cgpo_actor_loss_from_model_outputs(
    ...
    """Loss function for cgpo actor step, all inputs should be splitted into
    ...
    cgpo_loss_mask = input_.data["cgpo_loss_mask"]
    ...
    n_tokens = cgpo_loss_mask.count_nonzero()
    logprobs = gather_packed_shifted_log_probs(
        logits, cu_seqlens, packed_input_ids
    ).float()
    loss, cgpo_stat = ppo_functional.actor_loss_fn(
        logprobs=logprobs,
        old_logprobs=old_logp,
        advantages=advantages,
        eps_clip=eps_clip,
        loss_mask=cgpo_loss_mask,
    )

    importance_weight = cgpo_stat["importance_weight"].float() * n_tokens
    clip_ratio = cgpo_stat["clip_ratio"].float() * n_tokens
    approx_kl = cgpo_stat["approx_kl"].float() * n_tokens

    # Logging and early stopping according to KL (logp vs ref) or importance ratio (new logp vs old logp).
    mean_ref_kl = (kl_rewards.detach().float() * cgpo_loss_mask).sum()
    logging_loss = torch.where(cgpo_loss_mask, loss.detach().float(), 0.0).sum()
...
class CGPOActorInterface(model_api.ModelInterface):
    n_minibatches: int = 4
    ...
    def __post_init__(self):
        ...
        assert self.n_minibatches == 1, "CGPO only supports 1 minibatch"
    ...
    def inference(
        ...
        # Run mini-batched CGPO training!
        ...
        for data in datas:
            ...
            stats = module.train_batch(
                ...
                loss_fn=functools.partial(
                    _cgpo_actor_loss_from_model_outputs,
    ...
    def train_step(
        ...
        assert not self.just_after_backward, "In CGPOActorInterface, you couldn't call TRAIN_STEP(backward) twice without INFERENCE(step)"
        ...
        # FIXME: It only logs the MoE aux loss of the final CGPO mini-batch.
        global_stats.update(
        ...
        if train_stats:
            train_stats = dict(
                cgpo_approx_kl=float(train_stats["cgpo_approx_kl"] / _n_tokens),
        ...
        return SequenceSample.from_default(
            ...
            ids=input_.ids,
            data=dict(backward_signal=torch.zeros_like(input_lens)),
        )
...
model_api.register_interface("cgpo_actor", CGPOActorInterface)
